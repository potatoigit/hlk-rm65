...
int scanResult(lua_State *L)
{
	...
	if(snprintf(wrq.ifr_name, sizeof(wrq.ifr_name), "%s", interface) < 0){
		return -1;
	}
	if(snprintf(data, data_len, "%s", tmp_idx) < 0){
		return -1;
	}
...
int get_macaddr(lua_State *L)
{
	...
	if(snprintf(ifr.ifr_name, IF_NAMESIZE, "%s", ifname) < 0){
		return -1;
	}
...
int getApPin(lua_State *L)
{
	char new_pin[9];
	const char *interface = luaL_checkstring(L, 1);
	if(snprintf(new_pin, sizeof(new_pin), "%08d", get_ap_pin(interface)) < 0){
		return -1;
	}
...
int getWMOde(lua_State *L)
{
	char w_mode[5];
	const char *interface = luaL_checkstring(L, 1);
	if(snprintf(w_mode, sizeof(w_mode), "%d", get_w_mode(interface)) < 0){
		return -1;
	}
	lua_newtable(L);
...
int getCurrentWscProfile(lua_State *L)
{
	...
	if(snprintf((char *)data, sizeof(WSC_CONFIGURED_VALUE), "%s", "get_wsc_profile") < 0){
		return -1;
	}
	if(snprintf(wrq.ifr_name, sizeof(wrq.ifr_name), "%s", interface) < 0){
		return -1;
...
int OidQueryInformation(unsigned long OidQueryCode, int socket_id, const char *DeviceName, void *ptr, unsigned long PtrLength)
{
	...
	if(snprintf(wrq.ifr_name, sizeof(wrq.ifr_name), "%s", DeviceName) < 0){
		return -1;
	}
...
static unsigned int apcli_get_pincode_ioctl(const char *interface)
{
	...
	if( ioctl(socket_id, RT_PRIV_IOCTL, &wrq) == -1)
		perror("RT_PRIV_IOCTL ioctl error");
	close(socket_id);
...
int apcli_wps_get_pincode(lua_State *L)
{
	...
	if(snprintf(new_pin, sizeof(new_pin), "%08d", apcli_get_pincode_ioctl(interface)) < 0){
		return -1;
	}
...
int port_secured(const char *ifname)
{
	...
	if (OidQueryInformation(OID_GEN_MEDIA_CONNECT_STATUS, s, ifname, &ConnectStatus, sizeof(ConnectStatus)) < 0) {
		perror("Query OID_GEN_MEDIA_CONNECT_STATUS error!");
		close(s);
		return -1;
	}
	close(s);
...
int StaInfo(lua_State *L)
{
	...
	if(snprintf(iwr.ifr_name, IFNAMSIZ, "%s", interface) < 0){
		return -1;
	}
	...
	for (i = 0; i < table->Num; i++) {
		...
		// MAC Address
		if(snprintf(tmpBuff, sizeof(tmpBuff), "%02X:%02X:%02X:%02X:%02X:%02X", pe->Addr[0], pe->Addr[1], pe->Addr[2], pe->Addr[3],
				pe->Addr[4], pe->Addr[5]) < 0){
			return -1;
		}
		...
		if(snprintf(tmpBuff, sizeof(tmpBuff), "%d", pe->Aid) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "Aid");

		if(snprintf(tmpBuff, sizeof(tmpBuff), "%d", pe->Psm) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "Psm");

		if(snprintf(tmpBuff, sizeof(tmpBuff), "%d", pe->MimoPs) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		...
		if (pe->TxRate.field.MODE == 4){
			...
			vht_mcs = vht_mcs & 0xF;
			if(snprintf(tmpBuff, sizeof(tmpBuff), "%dS-M%d/", vht_nss, vht_mcs) < 0){
				return -1;
			}
			lua_pushstring(L, tmpBuff);
			lua_setfield(L, -2, "Mcs");
		} else{
			if(snprintf(tmpBuff, sizeof(tmpBuff), "%d", pe->TxRate.field.MCS) < 0){
				return -1;
			}
			lua_pushstring(L, tmpBuff);
			lua_setfield(L, -2, "Mcs");
		...
		if (pe->TxRate.field.BW == 0){
			if(snprintf(tmpBuff, sizeof(tmpBuff), "%d", 20) < 0){
				return -1;
			}
			lua_pushstring(L, tmpBuff);
			lua_setfield(L, -2, "Bw");
		} else if (pe->TxRate.field.BW == 1){
			if(snprintf(tmpBuff, sizeof(tmpBuff), "%d", 40) < 0){
				return -1;
			}
			lua_pushstring(L, tmpBuff);
			lua_setfield(L, -2, "Bw");
		} else if (pe->TxRate.field.BW == 2){
			if(snprintf(tmpBuff, sizeof(tmpBuff), "%d", 80) < 0){
				return -1;
			}
			lua_pushstring(L, tmpBuff);
			lua_setfield(L, -2, "Bw");
		...
		if(snprintf(tmpBuff, sizeof(tmpBuff), "%c", pe->TxRate.field.ShortGI? 'S': 'L') < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "Gi");

		if(snprintf(tmpBuff, sizeof(tmpBuff), "%s", phyMode[pe->TxRate.field.MODE]) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "PhyMode");

		if(snprintf(tmpBuff, sizeof(tmpBuff), "%s", pe->TxRate.field.STBC? "STBC": " ") < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		...
		// TxBF configuration
		if(snprintf(tmpBuff, sizeof(tmpBuff), "%c", pe->TxRate.field.iTxBF? 'I': '-') < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "iTxBF");

		if(snprintf(tmpBuff, sizeof(tmpBuff), "%c", pe->TxRate.field.eTxBF? 'E': '-') < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		...
		// RSSI
		if(snprintf(tmpBuff, sizeof(tmpBuff), "%d", (int)(pe->AvgRssi0)) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "AvgRssi0");

		if(snprintf(tmpBuff, sizeof(tmpBuff), "%d", (int)(pe->AvgRssi1)) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "AvgRssi1");

		if(snprintf(tmpBuff, sizeof(tmpBuff), "%d", (int)(pe->AvgRssi2)) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "AvgRssi2");

		if(snprintf(tmpBuff, sizeof(tmpBuff), "%d", (int)(pe->AvgRssi3)) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		...
		// Per Stream SNR
		if(snprintf(tmpBuff, sizeof(tmpBuff), "%0.1f", pe->StreamSnr[0]*0.25) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "StreamSnr0");
		//mcs>7? pe->StreamSnr[1]*0.25: 0.0);
		if(snprintf(tmpBuff, sizeof(tmpBuff), "%0.1f", pe->StreamSnr[1]*0.25) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "StreamSnr1");
		//mcs>15? pe->StreamSnr[2]*0.25: 0.0);
		if(snprintf(tmpBuff, sizeof(tmpBuff), "%0.1f", pe->StreamSnr[2]*0.25) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		...
		if (pe->TxRate.field.eTxBF) {
			if(snprintf(tmpBuff, sizeof(tmpBuff), "%0.1f", pe->SoundingRespSnr[0]*0.25) < 0){
				return -1;
			}
			lua_pushstring(L, tmpBuff);
			lua_setfield(L, -2, "SoundingRespSnr0");

			if(snprintf(tmpBuff, sizeof(tmpBuff), "%0.1f", pe->SoundingRespSnr[1]*0.25) < 0){
				return -1;
			}
			lua_pushstring(L, tmpBuff);
			lua_setfield(L, -2, "SoundingRespSnr1");

			if(snprintf(tmpBuff, sizeof(tmpBuff), "%0.1f", pe->SoundingRespSnr[2]*0.25) < 0){
				return -1;
			}
			lua_pushstring(L, tmpBuff);
		...
		if (((lastRxRate>>13) & 0x7) == 4){
			...
			if(snprintf(tmpBuff, sizeof(tmpBuff), "%dS-M%d", vht_nss_r, vht_mcs_r) < 0){
				return -1;
			}
			lua_pushstring(L, tmpBuff);
			lua_setfield(L, -2, "LastMcs");
		} else{
			if(snprintf(tmpBuff, sizeof(tmpBuff), "%d", mcs) < 0){
				return -1;
			}
			lua_pushstring(L, tmpBuff);
			lua_setfield(L, -2, "LastMcs");
		...
		if(snprintf(tmpBuff, sizeof(tmpBuff), "%c", ((lastRxRate>>8) & 0x1)? 'S': 'L') < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "LastGi");

		if(snprintf(tmpBuff, sizeof(tmpBuff), "%s", phyMode[(lastRxRate>>13) & 0x7]) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "LastPhyMode");

		if(snprintf(tmpBuff, sizeof(tmpBuff), "%s", ((lastRxRate>>9) & 0x3)? "STBC": " ") < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		...
		// Connect time
		if(snprintf(tmpBuff, sizeof(tmpBuff), "%02d", hr) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "Hr");

		if(snprintf(tmpBuff, sizeof(tmpBuff), "%02d", min) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
		lua_setfield(L, -2, "Min");

		if(snprintf(tmpBuff, sizeof(tmpBuff), "%02d", sec) < 0){
			return -1;
		}
		lua_pushstring(L, tmpBuff);
